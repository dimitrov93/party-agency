...
const Edit = () => {
  ...
  useEffect(() => {
    axios
      .get(`${URL.BASE_URL}/uploads/images/${type}/${album}/${oldAlbumName}/`, {
        headers: {
  ...
  }, [type, album, oldAlbumName]);

  const onUpdate = (e) => {
    try {
      axios
        .put(
          `${URL.BASE_URL}/uploads/images/${type}/${album}/${oldAlbumName}/`,
          { newAlbumName: title }
        )
        .then((res) => {
  ...
  return (
    <div className="container">
      <div className="edit__ctn">
        ...
        />
        <button className="edit__btn" onClick={onUpdate}>
          Update
        </button>
      </div>
    </div>
